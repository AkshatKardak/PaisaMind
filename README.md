...
### Core Capabilities:
* **Versioned Indian Tax Intelligence (Zero Hardcoded Logic):** Dynamic tax rule engine supporting FY 2024-25, FY 2025-26, and FY 2026-27 with official CBDT Gazette provenance, paired with a Section 44ADA legal evaluator enforcing entity compliance, specified profession codes, and digital receipt thresholds (₹75L vs ₹50L) with explicit `INSUFFICIENT_INFORMATION` validation.
* **3-Way Statutory TDS Reconciliation Engine:** Full 3-way matching connecting Invoices $\leftrightarrow$ Expected TDS $\leftrightarrow$ Bank Receipts $\leftrightarrow$ Form 26AS / AIS / Form 16A credits across all 9 statutory statuses (`MATCHED`, `PARTIAL_MATCH`, `TDS_MISMATCH`, `BANK_MISMATCH`, `MISSING_TDS`, `MISSING_PAYMENT`, `DUPLICATE`, `REVIEW_REQUIRED`, `UNRESOLVED`) with token-overlap entity matching.
...
* **Secure Streaming Document Pipeline & Magic-Byte Validation:** Page-by-page table extraction handling 100+ page statements with zero character truncation, retaining `sourcePage` and `documentId` traceability, pre-parser magic-byte verification (`%PDF-`, `PK\x03\x04`, CSV, PNG, JPEG) preventing file spoofing, and automated Indian PII masking (PAN, Aadhaar, bank accounts, payment cards).
* **Backend-Only Machine Learning Subsystem & Model Registry:** `ModelRegistry` governance tracking model versions, hyperparameters, and rolling backtest metrics (`MAE`, `RMSE`, `MAPE`), Modified Z-Score (MAD) spending outlier detection labeled `"Unusual transaction detected"`, and hybrid categorization with user feedback loops.
* **Volatility-Driven What-If Simulator & 5-Percentile Monte Carlo:** Simulates capital purchases and revenue drops scaled by historical income volatility ($\sigma$) and 100-iteration Monte Carlo distributions (`P10`, `P25`, `P50`, `P75`, `P90`) with exact probabilities of negative cash balance and runway exhaustion.
* **Configurable & Explainable Financial Health Engine:** 8-component transparent health score exposing `rawMetric`, `normalizedMetric`, `weight`, `score`, `reason`, and `confidence`.
...
* **Grounded AI Financial Copilot (IDOR-Protected):** Powered by Groq LLaMA-3.3 with 21 server-side deterministic tool engines. Strict session-level `userId` scoping prevents IDOR, while inputs are sanitized against prompt injection.
...
| **TDS Reconciliation** | Not supported | Manual tick-mark matching | Cannot cross-reference bank & 26AS | **Automated 3-Way Reconciliation (Invoice $\leftrightarrow$ Bank $\leftrightarrow$ 26AS) across 9 Statutory Statuses** |
| **Document Ingestion & Security** | Manual CSV upload | Requires bank netbanking aggregation feed | Not supported | **Magic-Byte Signature Verification + Streaming Page-by-Page Extraction (Zero Truncation) + PII Masking** |
...
| **Cash Flow Forecasting & Governance** | Historical lookback only | Static cash-flow statement | Guesses future scenarios | **Backend ML (Holt-Linear Trend, Rolling Backtest MAE/RMSE/MAPE) + ModelRegistry** |
| **Spending Anomaly Detection** | Simple threshold alerts | Basic variance reports | No statistical rigor | **Modified Z-Score using MAD + Outlier Scoring labeled "Unusual transaction detected"** |
| **Scenario Stress-Testing** | Not supported | Static manual spreadsheets | Uncalibrated subjective answers | **Historical Volatility Shocks + 5-Percentile Monte Carlo (`P10`, `P25`, `P50`, `P75`, `P90`) + Cash Risk %** |
| **Financial Health Engine** | Not supported | Basic ratio view | Subjective rating | **Configurable HealthConfig + Component Breakdown (`rawMetric`, `normalizedMetric`, `reason`, `confidence`)** |
