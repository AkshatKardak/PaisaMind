...
const calculateDetailedHealthScore = async (userId) => {
  ...
  const overallResult = {
    ...
    weightsUsed: weights,
    breakdown: {
      cashFlowStability: { score: scoreCashFlow, max: weights.cashFlowStability, label: "Cash Flow Stability", details: `${volatilityData.stabilityLabel}` },
      savingsRate: { score: scoreSavings, max: weights.savingsRate, label: "Savings Rate", details: `${Math.round(effectiveSavingsRate * 100)}% 3-month savings rate` },
      expenseControl: { score: scoreExpenses, max: weights.expenseControl, label: "Expense Control", details: `${Math.round(expenseRatio * 100)}% of income spent` },
      emergencyRunway: { score: scoreRunway, max: weights.emergencyRunway, label: "Emergency Runway", details: `${runwayMonths} months available` },
      invoiceReliability: { score: scoreInvoice, max: weights.invoiceReliability, label: "Invoice Reliability", details: `${invoiceRiskData.clients?.length || 0} active clients` },
      debtBurden: { score: scoreDebt, max: weights.debtBurden, label: "Debt Burden", details: `${dti}% Debt-to-Income` },
      clientConcentration: { score: scoreConcentration, max: weights.clientConcentration, label: "Client Concentration", details: `${topClientPct}% max client share` },
      taxReserveCoverage: { score: scoreTaxReserve, max: weights.taxReserveCoverage, label: "Tax Reserve Coverage", details: "Sec 44ADA / New Regime Reserve" },
    },
    improvements,
